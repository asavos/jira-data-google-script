function JiraDataGoogleScript() {
    ...
    this.queryJira = function (path) {
        ...
        if (responseCode === 401) {
            ...
            Browser.msgBox('Not authorised with Jira - try setting the correct username and password in Jira > Set credentials...');
            return '';
        } else if (responseCode !== 200) {

            Browser.msgBox('Unexpected error fetching data from Jira API ' + responseCode);
            return '';
        }
    ...
    this.fetchTicketsFromJira = function () {
        ...
        return JSON.parse(this.queryJira('search?jql=project%20%3D%20%22' + project + '%22%20and%20status%20%3D%20%22' + closedStatus + '%22%20type%20in%20(' + ticketTypes + ')%20and%20status%20was%20%22' + qualifyingStatus + '%22%20resolutionDate%20%3E%20%22' + startDate + '%22%20and%20resolutionDate%20%3C%20%22' + endDate + '%22%20order%20by%20resolutionDate%20DESC&expand=changelog&maxResults=' + this.maxResults));
    };

    this.getFieldsFromJira = function () {

        return JSON.parse(this.queryJira('field'));
    };

    this.getFields = function () {

        var jiraFields = this.getFieldsFromJira(),
            sortedFields = {
        ...
        for (i = 0; i < jiraFields.length; i = i + 1) {

            sortedFields.ids[i] = jiraFields[i].id;
            sortedFields.names[i] = jiraFields[i].name.toLowerCase();
        }
...
// }
//
// function getStories() { - converted to fetchTicketsFromJira()
//     var allData = {issues: []};
